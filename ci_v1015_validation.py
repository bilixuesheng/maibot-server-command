...
def test_manifest_and_components() -> None:
    ...
    matching_config = plugin.build_default_config()
    matching_config["root_mode"].update(
        {
            "enabled": True,
            "confirmation_1": True,
            "confirmation_2": True,
    ...
    matching_config["unrestricted_root"].update(
        {
            "enabled": True,
            "confirmation_1": True,
            "confirmation_2": True,
    ...
    plugin.set_plugin_config(matching_config)
    restricted_active, _ = plugin._root_mode_state()
    assert restricted_active
    assert plugin._unrestricted_root_state(restricted_active)[0]

    all_switches_enabled = plugin.build_default_config()
    all_switches_enabled["root_mode"].update(matching_config["root_mode"])
    all_switches_enabled["unrestricted_root"].update(
        matching_config["unrestricted_root"]
    )
    for index in range(1, 9):
        all_switches_enabled["unrestricted_root"][f"confirmation_{index}"] = True
    plugin.set_plugin_config(all_switches_enabled)
    assert not plugin._unrestricted_root_state(True)[0]

    all_switches_disabled = plugin.build_default_config()
    all_switches_disabled["root_mode"].update(matching_config["root_mode"])
    all_switches_disabled["unrestricted_root"].update(
        matching_config["unrestricted_root"]
    )
    for index in range(1, 9):
        all_switches_disabled["unrestricted_root"][f"confirmation_{index}"] = False
    plugin.set_plugin_config(all_switches_disabled)
    assert not plugin._unrestricted_root_state(True)[0]
